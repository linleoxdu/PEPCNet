...
class DRStackedConvBlocks(nn.Module):
    def __init__(self,
                 num_convs: int,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
        ...
        self.first_conv = ConvDropoutNormReLU(
                conv_op, input_channels, output_channels[0], kernel_size, initial_stride, conv_bias, norm_op,
                norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, nonlin_first
            )
        self.convs = nn.Sequential(
    ...
    def forward(self, x):
        middle_x = self.first_conv(x)
        return middle_x, self.convs(middle_x)
...
class ConvDropoutNormReLUWOPADDING(nn.Module):
    def __init__(self,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
                 output_channels: int,
        ...
        super(ConvDropoutNormReLUWOPADDING, self).__init__()
        self.input_channels = input_channels
        self.output_channels = output_channels
        stride = maybe_convert_scalar_to_list(conv_op, stride)
        self.stride = stride
